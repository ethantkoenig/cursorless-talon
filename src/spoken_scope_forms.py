...
def create_flattened_talon_list(ctx: Context, graphemes_talon_list: dict[str, str]):
    ...
    for list_name, prefix in lists_to_merge.items():
        for key, value in ctx.lists[f"user.{list_name}"].items():
            ...
            if prefix == "surroundingPair":
                # "wrap" added to avoid ambiguity. Since "round" and friends are
                # also used for closing delimiter characters, a command like
                # "change round dot" could be interpreted as "(change round)
                # dot" or "change (round dot)".
                scope_types_singular[f"{key} wrap"] = f"{prefix}.{value}"
            else:
        ...
        for key, value in ctx.lists[f"user.{list_name}_plural"].items():
            ...
            # We don't make a similar change here, since this case doesn't
            # suffer from the same ambiguity, and something like "rounds wrap"
            # or "rounds wraps" sounds weird.
